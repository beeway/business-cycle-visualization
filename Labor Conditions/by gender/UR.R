...
library(tidyverse); ur=read.csv("URS.csv")
...
ura=ur$UR; urm=ur$URM; urw=ur$URW
md=data.frame(date, ura, urm, urw)

library(ggplot2); library(scales)

# Year on X-axis
ggplot(md, aes(x=date)) + geom_line(aes(y=ura, col="ura")) + 
  geom_line(aes(y=urm, col="urm")) + geom_line(aes(y=urw, col="urw")) + theme_bw() +
    ...
    scale_color_discrete(labels=c("Unemployment Rate (%)", "Unemployment Rate - Men", "Unemployment Rate - Women")) 
...
g = ggplot(md, aes(x=date)) + geom_line(aes(y=ura, col="ura")) + 
  geom_line(aes(y=urm, col="urm")) + geom_line(aes(y=urw, col="urw")) + theme_bw() +
  labs(x="", y="", caption="Source: Biwei Chen (2022). Data: BLS; NBER.") + 
  scale_y_continuous(limits = c(0,15), breaks=seq(0,15,1), sec.axis=dup_axis(name=""), expand=c(0,0)) +
  scale_x_date(date_labels="%Y",date_breaks = "1 years", date_minor_breaks="1 years", expand=c(0.01,0.01)) + 
  theme(axis.text.x=element_text(angle=90, vjust=0.5, size = 9))  +
  theme(legend.title = element_blank(), legend.position = c(0.15, 0.9)) +
  scale_color_discrete(labels=c("Unemployment Rate (%)", "Unemployment Rate - Men", "Unemployment Rate - Women")) 
