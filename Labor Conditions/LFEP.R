...
library(tidyverse); d1=read.csv("All.csv")
...
ur=d1$UR; lr=d1$LRP; er=d1$EPR
d2=data.frame(date, ur, lr, er)

library(ggplot2)

# Year on X-axis
ggplot(d2, aes(x=date)) + geom_line(aes(y=lr, col="lr")) + geom_line(aes(y=er, col="er")) + theme_bw() +
  ...
  scale_y_continuous(limits = c(50, 70), breaks=seq(50,70, 1), sec.axis=dup_axis(name="")) +
  ...
  scale_color_discrete(labels=c("Labor Force Participation (%)", "Employment-to-Population (%)")) 
...
g=ggplot(d2, aes(x=date)) + geom_line(aes(y=lr, col="lr")) + geom_line(aes(y=er, col="er")) + theme_bw() +
  theme(legend.title = element_blank(), legend.position = "none", axis.title.y.right = element_text()) +
  labs(x="", y="", caption="Source: Biwei Chen (2022). Data: BLS; FRED.") + 
  ...
  g + annotate("rect", xmin=xl, xmax=xr, ymin =rep(50,12), ymax =rep(70,12), alpha = .45) 
